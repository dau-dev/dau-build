...
from dau_core.registers import DEFAULT_STREAM_JOB_REGISTER_CONTRACT, StreamJobRegisterContract
...
class MmJobShellRequest:
    """Inputs for generating the MM job shell project."""

    output_root: Path
    hdl_sources: tuple[Path, ...]
    generated_top_path: Path
    top_module: str = "dau_mm_stream_job"
    # "record-batch" emits the single-tile MM job top; "ohlcv" emits the
    # composed pipeline plus its MM job top
    variant: str = "record-batch"
    part: str = DPV1_PART
    register_contract: StreamJobRegisterContract = field(default_factory=lambda: DEFAULT_STREAM_JOB_REGISTER_CONTRACT)
...
def mm_job_shell_project_tcl(request: MmJobShellRequest) -> str:
    ...
    generated_sources = (
        (request.output_root / "dau_ohlcv_pipeline.v", request.generated_top_path) if request.variant == "ohlcv" else (request.generated_top_path,)
    )
    sources = " \\\n".join(f'    "{path.as_posix()}"' for path in (*request.hdl_sources, *generated_sources))
    sv_typing = "\n".join(
        f'set_property file_type SystemVerilog [get_files "{path.as_posix()}"]' for path in request.hdl_sources if path.suffix == ".sv"
    )
    contract = request.register_contract
    swizzle_pairs = " ".join(f"{lane} {channel}" for lane, channel in GT_LANE_SWIZZLE)
    return f"""# GENERATED by dau_build.dpv1_shell — do not edit.
...
assign_bd_address -offset 0x{contract.input_buffer_address:08X} -range 0x{request.input_bram_bytes:08X} -target_address_space [get_bd_addr_spaces xdma_0/M_AXI] [get_bd_addr_segs bram_ctrl_in/S_AXI/*] -force
assign_bd_address -offset 0x{contract.output_buffer_address:08X} -range 0x{request.output_bram_bytes:08X} -target_address_space [get_bd_addr_spaces xdma_0/M_AXI] [get_bd_addr_segs bram_ctrl_out/S_AXI/*] -force
assign_bd_address -offset 0x{contract.register_window_offset:08X} -range 0x00001000 -target_address_space [get_bd_addr_spaces xdma_0/M_AXI_LITE] [get_bd_addr_segs {request.top_module}_0/S_AXI/*] -force
...
def write_mm_job_shell_artifacts(request: MmJobShellRequest) -> tuple[Path, ...]:
    """Write the generated top, project Tcl, constraints, and swizzle hook
    into ``request.output_root``; returns the written paths."""
    from dau_build.shell_tops import generate_mm_stream_job_top_sv

    request.output_root.mkdir(parents=True, exist_ok=True)
    written = []

    top_path = request.output_root / request.generated_top_path.name
    if request.variant == "ohlcv":
        from dau_build.shell_tops import generate_mm_ohlcv_job_top_sv, generate_ohlcv_pipeline_top_sv

        pipeline_path = request.output_root / "dau_ohlcv_pipeline.v"
        pipeline_path.write_text(generate_ohlcv_pipeline_top_sv())
        written.append(pipeline_path)
        top_path.write_text(generate_mm_ohlcv_job_top_sv(module_name=request.top_module))
    else:
        top_path.write_text(generate_mm_stream_job_top_sv(module_name=request.top_module))
