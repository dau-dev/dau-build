...
class VivadoOverlayDefinition(BaseModel):
    ...
    fields. dau-build packages no payload of its own: without an injected
    definition the staged overlay is overlay-less (no HDL grafts, no
    block-design edit, no lane placements) — the same pattern as the
    injectable ``smoke_command``."""
...
def dau_overlay_manifest(
    ...
    *,
    ...
    definition = overlay_definition if overlay_definition is not None else VivadoOverlayDefinition()
...
def vivado_build_tcl(
    *,
    ...
    placements = lane_placements if lane_placements is not None else ()
