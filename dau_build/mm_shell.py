...
def _project_preamble_tcl(request, *, banner: str) -> str:
    ...
    # A tile that `include`s a header needs that header IN THE PROJECT, not
    # merely on an include path: Vivado refuses with "Given include File
    # ... needs to be addded to the project in order to use it as an RTL
    # module" and the block design fails at create_bd_cell. Headers are not
    # cores, so they never appear in hdl_sources -- collect them from the
    # directories the sources come from, which is the same rule the OOC
    # synthesis and the verilator benches already follow.
    headers = sorted(
        {header for source in request.hdl_sources for header in source.parent.glob("*.svh")},
        key=lambda path: path.as_posix(),
    )
    sources = " \\\n".join(f'    "{path.as_posix()}"' for path in (*request.hdl_sources, *headers, *generated_paths))
    sv_typing = "\n".join(
        f'set_property file_type SystemVerilog [get_files "{path.as_posix()}"]' for path in request.hdl_sources if path.suffix == ".sv"
    )
    # `is_global_include` is what makes the header visible to every source in
    # the fileset rather than only where an explicit include path reaches.
    header_typing = "\n".join(
        f'set_property file_type "Verilog Header" [get_files "{path.as_posix()}"]\n'
        f'set_property is_global_include true [get_files "{path.as_posix()}"]'
        for path in headers
    )
    if header_typing:
        sv_typing = f"{sv_typing}\n{header_typing}" if sv_typing else header_typing
    return f"""# GENERATED by dau_build.mm_shell — do not edit.
{banner}
