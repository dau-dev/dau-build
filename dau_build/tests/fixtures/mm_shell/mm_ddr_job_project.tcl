...
create_project -force project_mm "$origin_dir/project_mm" -part xc7k325tffg676-2
...
set_property -dict [list \
    CONFIG.XML_INPUT_FILE "$origin_dir/probe_mig.prj" \
...
foreach cell [get_cells -hierarchical -quiet -filter {REF_NAME == GTXE2_CHANNEL || ORIG_REF_NAME == GTXE2_CHANNEL}] {
    if {[regexp {pipe_lane\[(\d+)\]} $cell -> lane]} {
        dict set routed_lanes $lane [get_property LOC $cell]
    }
}
foreach {lane channel} {7 GTXE2_CHANNEL_X0Y7 6 GTXE2_CHANNEL_X0Y6 5 GTXE2_CHANNEL_X0Y5 4 GTXE2_CHANNEL_X0Y4 3 GTXE2_CHANNEL_X0Y3 2 GTXE2_CHANNEL_X0Y2 1 GTXE2_CHANNEL_X0Y1 0 GTXE2_CHANNEL_X0Y0} {
