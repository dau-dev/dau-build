...
def test_platform_threads_through_the_ddr_project(tmp_path: Path) -> None:
    ...
    request = _ddr_request(tmp_path).model_copy(update={"platform": platform, "part": platform.part})
...
def test_request_part_defaults_from_the_platform(tmp_path: Path) -> None:
    tile = tmp_path / "stream_tile.sv"
    tile.write_text("module stream_tile; endmodule\n")
    common = dict(
        output_root=tmp_path / "shell",
    ...
    assert MmJobShellRequest(**common).part == "xc7a200tfbg484-2"  # dpv1 default
    assert MmJobShellRequest(platform=_probe_platform(), **common).part == "xc7k325tffg900-2"
    assert MmJobShellRequest(platform=_probe_platform(), part="xc7k325tffg676-1", **common).part == "xc7k325tffg676-1"
