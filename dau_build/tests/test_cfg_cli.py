...
def test_cfg_cli_runs_profile_only_simulate(tmp_path: Path, capsys) -> None:
    exit_code = main(
        [
            ...
            "model.module=''",
            "model.profile=dau-int32-record-batch-aggregation",
    ...
    assert "profile=dau-int32-record-batch-aggregation" in captured.out
...
def test_cfg_cli_open_registration_via_config_dir_overlay(tmp_path: Path, capsys) -> None:
    ...
    (overlay / "task" / "smoke-sim.yaml").write_text(
        "\n".join(
            (
                ...
                "module: ''",
                "simulator: verilator",
                "profile: dau-int32-predicate-filter",
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert "profile=dau-int32-predicate-filter" in captured.out
    assert "status=passed" in captured.out
